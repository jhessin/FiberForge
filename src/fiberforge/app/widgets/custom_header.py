    ...
    def render(self) -> RenderResult:
        ...
        if self.time_clock:
            total_minutes, seconds = divmod(
                self.time_clock.time_today.total_seconds(), 60
            )
            hours, minutes = divmod(total_minutes, 60)
            return Text(f'{int(hours):02}:{int(minutes):02}:{int(seconds):02}::{clock}')
        return Text(clock)
...
class Header(Widget):
    ...
    time_clock: Reactive[Optional[TimeClock]] = Reactive(TimeClock)
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            show_clock: ``True`` if the clock should be shown on the right of
            the header.
            ...
            time_format: Time format (used by strftime) for clock, or `None`
            for default.
        """
        ...
        with Database() as db:
            self.set_reactive(Header.time_clock, db.clock.today())

    def compose(self) -> ComposeResult:
        yield HeaderIcon().data_bind(Header.icon)
        yield HeaderTitle()
        yield (
            HeaderClock().data_bind(Header.time_format, Header.time_clock)
            if self._show_clock
    ...
    def format_title(self) -> Content:
        ...
        Override this method if you want to customize how the title is
    ...
    def screen_sub_title(self) -> str:
        ...
        This depends on [`Screen.sub_title`][textual.screen.Screen.sub_title]
        and [`App.sub_title`][textual.app.App.sub_title].
        """
